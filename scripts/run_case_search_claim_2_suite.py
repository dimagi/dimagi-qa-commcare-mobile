...
Claim 2" (row 20) - see scripts/appium_case_search_claim_2_scenarios.py's
own module docstring for the full citation.

Usage:
    python scripts/run_case_search_claim_2_suite.py
    python scripts/run_case_search_claim_2_suite.py --release-tag commcare_2.64.1 --devices "Samsung Galaxy S26-16.0"
"""
import argparse
import json
import os
...
import appium_case_search_claim_2_scenarios as cscl2
...
def main():
    ...
    parser.add_argument("--build-name", default="QA-COMMCARE-MOBILE-appium-case-search-claim-2")
    ...
    print("Running Case Search & Claim 2 ...")
    start = time.monotonic()
    driver = None
    result = None
    try:
        driver = bs.start_session(app_url, device, os_version, build_name=args.build_name,
                                   session_name="case_search_claim_2")
        completed, captured = cscl2.run_case_search_claim_2(driver, app_code, cc_username, cc_password)
        print(f"  Song used: {captured.get('song')!r}, Query outcome: {captured.get('result')!r}")
        result = report_generator.TestResult(
            name="case_filters/case_search_claim_2",
        ...
        _save_failure_evidence(driver, "case_search_claim_2")
        result = report_generator.TestResult(
            name="case_filters/case_search_claim_2",
            workflow="case_filters",
            status="failed",
            ...
            failed_step=f"case_search_claim_2 (Appium) - {exc.step_name}: {exc.original}",
        )
        ...
        result = report_generator.TestResult(
            ...
            device=f"{device}-{os_version}",
            error=str(exc),
            failed_step=f"case_search_claim_2 (Appium) - session/infra error: {exc}",
        )
    finally:
        # Same best-effort cleanup precedent as run_case_search_claim_suite.py's
        # own 2026-09-16 fix - a transient network hiccup during driver.quit()/
        # session-status reporting must never mask the real scenario result.
    ...
    print(f"  case_search_claim_2: {result.status}"
    ...
    build_id = f"case-search-claim-2-{int(time.time())}"
