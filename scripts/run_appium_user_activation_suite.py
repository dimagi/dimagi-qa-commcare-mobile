"""
CLI runner for Master Mobile Plan (2026) > Form Submissions > "Active users
login" / "Inactive users login" / "Deactivated user login" (rows 60-62) -
see scripts/appium_user_activation_scenarios.py's own module docstring for
the full citation on why Appium (not Maestro) and what "maestro_inactive"
is for.

Usage:
    python scripts/run_appium_user_activation_suite.py
    python scripts/run_appium_user_activation_suite.py --release-tag commcare_2.64.1 --devices "Samsung Galaxy S26-16.0"
"""
import argparse
import json
import os
...
import appium_user_activation_scenarios as uas
...
def main():
    ...
    parser.add_argument("--build-name", default="QA-COMMCARE-MOBILE-appium-user-activation")
    ...
    print(f"Resolving install code for BASIC_TESTS ({domain}/{app_id}) ...")
    ...
    print(f"Resolving/creating {uas.MAESTRO_INACTIVE_USERNAME!r} mobile worker ...")
    user_id = hq.create_mobile_worker(uas.MAESTRO_INACTIVE_USERNAME, uas.MAESTRO_INACTIVE_PASSWORD)
    print(f"  user_id: {user_id}")

    mm_username = os.environ["HQ_MOBILE_WORKER_USERNAME"]
    mm_password = os.environ["HQ_MOBILE_WORKER_PASSWORD"]

    bs = AppiumBrowserStackClient()
    print(f"Uploading APK ({apk_path}) to BrowserStack ...")
    app_url = bs.upload_app(apk_path)["app_url"]

    print("Running Active users login ...")
    active_result = _run_scenario(
        bs, app_url, device, os_version, args.build_name, "active_users_login",
        "form_submissions/active_users_login",
        lambda driver: uas.run_active_users_login(driver, app_code, mm_username, mm_password),
    )
    print(f"  active_users_login: {active_result.status}"
          + (f" - {active_result.failed_step}" if active_result.status == "failed" else ""))

    print("Running Inactive users login ...")
    inactive_result = _run_scenario(
        bs, app_url, device, os_version, args.build_name, "inactive_users_login",
        "form_submissions/inactive_users_login",
        lambda driver: uas.run_inactive_users_login(driver, app_code, hq, user_id),
    )
    print(f"  inactive_users_login: {inactive_result.status}"
          + (f" - {inactive_result.failed_step}" if inactive_result.status == "failed" else ""))

    print("Running Deactivated user login ...")
    deactivated_result = _run_scenario(
        bs, app_url, device, os_version, args.build_name, "deactivated_user_login",
        "form_submissions/deactivated_user_login",
        lambda driver: uas.run_deactivated_user_login(driver, app_code, hq, user_id),
    )
    print(f"  deactivated_user_login: {deactivated_result.status}"
          + (f" - {deactivated_result.failed_step}" if deactivated_result.status == "failed" else ""))
    ...
    this_run_results = [active_result, inactive_result, deactivated_result]
    ...
    build_id = f"appium-user-activation-{int(time.time())}"
